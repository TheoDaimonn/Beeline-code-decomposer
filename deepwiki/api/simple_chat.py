...
async def chat_completions_stream(request: ChatCompletionRequest):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Raw repo_url repr: {repo_url}")
            repo_url = ''.join(ch for ch in repo_url if ch.isprintable())
            repo_url = repo_url.strip()
            logger.info(f"Cleaned repo_url repr: {repo_url}")

            request_rag.prepare_retriever(repo_url, access_token)
            logger.info(f"Retriever prepared for {repo_url}")

        except Exception as e:
            logger.error(f"Error preparing retriever: {str(e)}")
            raise HTTPException(status_code=500, detail=f"Error preparing retriever: {str(e.with_traceback())}")
                ...
                if "maximum context length" in error_message or "token limit" in error_message or "too many tokens" in error_message:
                    ...
                    try:
                        ...
                        fallback_response = client.chat.completions.create(
                            messages=[{"content":simplified_prompt, "role":"user"}],
