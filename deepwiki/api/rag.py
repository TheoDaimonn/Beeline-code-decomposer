...
class RAG(adal.Component):
    ...
    def __init__(self, use_s3: bool = False):  # noqa: F841 - use_s3 is kept for compatibility
        ...
        self.embedder = adal.Embedder(
            model_client=configs["embedder"]["model_client"](**configs["embedder"]["client_kwargs"]),
        ...
        self.generator = adal.Generator(
            ...
            model_client=configs["generator"]["model_client"](**configs["generator"]["client_kwargs"]),
    ...
    def prepare_retriever(self, repo_url_or_path: str, access_token: str = None):
        ...
        logger.info(f"[DEBUG] FAISS index ntotal={self.retriever.index.ntotal}, dimension={self.retriever.index.d}")
        for i, doc in enumerate(self.transformed_docs[:5]):
            logger.info(f"[DEBUG] doc[{i}] vector length = {len(doc.vector)}")
    ...
    def call(self, query: str) -> Tuple[Any, List]:
        ...
        """
        try:
            logger.info(f"[DEBUG] Calling retriever with query list: {[query]!r}")

            print('зашли в кол')
            try:
                retrieved_documents = self.retriever([query])
            except Exception:
                logger.exception("Ошибка при self.retriever(query)")
                raise
                
            logger.debug(f"[DEBUG] raw type: {type(retrieved_documents)}, repr: {repr(retrieved_documents)[:200]}")
            ...
            print('-------------------------- retrieve -------------------------------')
            logger.debug(f"[DEBUG] raw type: {type(retrieved_documents)}, raw repr: {repr(retrieved_documents)[:200]}")
